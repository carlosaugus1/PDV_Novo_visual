...
export const SalesModule = {
    ...
    async confirmPayment() {
        if (!store.state.ui.currentPaymentMethod) return uiUtils.notify('Selecione Pagamento', 'error');
        ...
        if (store.state.ui.editingOrderId) {
            import('./orders.js').then(module => module.OrdersModule.closeOrder(store.state.ui.editingOrderId));
            sale.isFromComanda = true;
        }

        if (!store.state.salesHistory[store.state.ui.today]) store.state.salesHistory[store.state.ui.today] = [];
        ...
        uiUtils.notify('Venda Finalizada!', 'success');
    },
    ...
    printThermalReceipt(sale) {
        ...
        const deliveryHtml = sale.deliveryInfo ? `<div class="divider"></div><div class="center">*** ENTREGA ***</div><div>${sale.deliveryInfo.name}</div><div>${sale.deliveryInfo.address}</div>` : '';

        const html = `
        <html><head><style>
            body { font-family: 'Courier New', monospace; font-size: 12px; margin: 0; width: 58mm; color: #000; font-weight: bold; }
            .center { text-align: center; }
            .line { display: flex; justify-content: space-between; }
            .divider { border-top: 2px solid #000; margin: 5px 0; }
            .big { font-size: 14px; font-weight: 800; }
        </style></head><body>
            <div class="center"><div class="big">REVVO MILK</div><div>Guaraciaba do Norte - CE</div></div>
            ...
            ${deliveryHtml}
            <div class="center"><br>Obrigado!</div>
    ...
    renderHistory() {
        ...
        if (sales.length === 0) list.innerHTML = '<div style="padding:15px; color:#888;">Sem vendas.</div>';
        
        sales.forEach(s => {
            t += s.total;
            const methodPT = Settings.paymentMap[s.paymentMethod] || s.paymentMethod;
            list.innerHTML += `<div class="sale-card" style="margin-bottom:10px; padding:10px; border:1px solid #eee; border-radius:8px; display:flex; justify-content:space-between;">
                <div><strong>R$ ${s.total.toFixed(2)}</strong><br><small>${s.date.split(' ')[1]} - ${methodPT}</small></div>
            </div>`;
        });
        document.getElementById('history-grand-total').textContent = `R$ ${t.toFixed(2)}`;
    },
    ...
    bindEvents() {
        ...
        document.getElementById('main-payment-options').addEventListener('click', (e) => {
            ...
            if (opt) {
                ...
                document.getElementById('cash-input').style.display = opt.dataset.method === 'cash' ? 'block' : 'none';
            }
        });
        ...
        document.getElementById('cash-received').addEventListener('input', (e) => {
            ...
            const sub = store.state.cart.reduce((a, b) => a + b.totalPrice, 0);
            const fee = (store.state.ui.deliveryMode === 'entrega') ? (parseFloat(document.getElementById('delivery-fee').value) || 0) : 0;
            document.getElementById('change-amount').textContent = (received - (sub + fee)).toFixed(2);
        });

        document.getElementById('history-date').addEventListener('change', () => this.renderHistory());
    }
};